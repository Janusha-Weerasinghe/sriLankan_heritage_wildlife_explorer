...
struct HeritageExplorationView: View {
    ...
    // Repository for heritage data
    private let heritageRepository = HeritageRepository.shared
    
    // Computed property for filtered sites
    private var filteredSites: [HeritageSite] {
        let allSites = heritageRepository.getAllHeritageSites()
        ...
        case "Popular":
            return allSites.filter { $0.isFavorite }
        case "Nearby":
            return allSites.sorted {
                ($0.distance ?? Double.infinity) < ($1.distance ?? Double.infinity)
            }
        case "Historical":
            return allSites.filter {
                $0.historicalPeriod.contains("Ancient") ||
                $0.historicalPeriod.contains("Medieval")
            }
        case "UNESCO":
            return allSites.filter {
                $0.culturalSignificance.contains("UNESCO")
            }
        default:
            return allSites
        }
        ...
        NavigationView {
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        FeaturedSiteView(
                            site: heritageRepository.getFeaturedSite(),
                            onARButtonTapped: {
                                selectedSite = heritageRepository.getFeaturedSite()
                                showARView = true
                            }
                        )
                        ...
                        PopularSitesView(
                            sites: filteredSites,
                            onSiteTapped: { site in
                                selectedSite = site
                            }
                        )
                        
                        MapExplorerView(sites: heritageRepository.getAllHeritageSites())
                    }
                    .padding(.bottom, 16)
                }
            }
